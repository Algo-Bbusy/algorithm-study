	...
	public static void main(String[] args) throws IOException {
		...
		if (gDir == 1 || gDir == 2) { // 동근이가 북 or 남쪽에 위치
			for (int i = 0; i < store; i++) {
				...
				if ((gDir == 1 && sDir == 2) || (gDir == 2 && sDir == 1)) minDistance += Math.min((width - gX) + height + (width - sX), gX + height + sX);
			...
			for (int i = 0; i < store; i++) {
				...
				if ((gDir == 3 && sDir == 4) || (gDir == 4 && sDir == 3)) minDistance += Math.min((height - gY) + width + (height - sY), gY + width + sY);
			}
		}
