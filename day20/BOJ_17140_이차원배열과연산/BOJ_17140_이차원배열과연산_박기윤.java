...
public class BOJ_17140_이차원배열과연산_박기윤 {
	...
	public static void main(String[] args) throws IOException {
			...
			if (rowSize >= colSize) { // R 연산 수행
				...
				for (int row = 0; row < rowSize; row++) {
					for (int col = 0, length = colSize; col < length; col++) {
						...
						if (map.get(cur) == null) map.put(cur, 1); // map에 해당 숫자를 Key로 갖는 값이 없다면 -> 최초 카운트
						else map.put(cur, map.get(cur) + 1); // 있다면 현재 count값 + 1
		...
		if (time > 100) bw.write("-1");
		else bw.write(Integer.toString(time));
		br.close();
		bw.flush();
		bw.close();
